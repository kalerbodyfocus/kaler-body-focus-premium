...
export function formatFullPhoneNumber(number: string, countryCode: string): string {
  ...
  try {
    ...
    // Fallback if not valid: clean number of dialcode if present, then combine
    ...
    let cleanNum = number.replace(/^\+/, "").trim();
    if (dial && cleanNum.startsWith(dial.replace(/^\+/, ""))) {
      cleanNum = cleanNum.substring(dial.replace(/^\+/, "").length).trim();
    }
